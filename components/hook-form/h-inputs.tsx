...
export const Switch = (props) => {
    ...
    return (
        <Form.Control id={id} {...register(name, rules)} {...props} label={label === true ? t(name) : t(label)} type="switch" />
    );
};
...
export const HField: FunctionComponent<Props> = ({
    ...
    const feedback = feedbackText && <Form.Control.Feedback className="d-block" type="invalid">{feedbackText}</Form.Control.Feedback>;
    ...
    return (
        <Form.Group {...restProps}>
            <Form.Label {...labelProps} htmlFor={id} className={classNames(labelProps?.className || '', { required: rules.required })}>
                {t(label === true ? name : label)}
            </Form.Label>
            {input}
            {feedback}
